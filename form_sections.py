# =================================================================
# form_sections.py - UI定義（API実行ガード強化）
# =================================================================
...
def parse_rm_coords(rm_str):
    ...
    try: return json.loads(rm_str.replace(" ", ""))
    except: return []
...
def render_form(api_service):
    ...
    st.session_state["タイプ"] = c1_1.selectbox("タイプ", constants.MASTER_DATA["types"], index=safe_index(constants.MASTER_DATA["types"], st.session_state.get("タイプ")))
    ...
    st.session_state["タイトル"] = c1_6.text_input("タイトル", value=st.session_state.get("タイトル", ""))
    ...
    st.session_state["24時間交通量_平日"] = c2_4.text_input("24H交通量", value=st.session_state.get("24時間交通量_平日", ""))
    ...
    if btn_col1.button("🔍 道路属性一括取得", use_container_width=True, type="primary"):
    ...
    if btn_col2.button("🛣️ 経路座標取得", use_container_width=True, type="secondary"):
        s_coords = clean_latlon(st.session_state.get("始点緯度経度"))
        e_coords = clean_latlon(st.session_state.get("終点緯度経度"))
        token = st.session_state.get("drm_token")
        if not token:
            st.warning("DRMログインが必要です。")
        elif not s_coords or not e_coords:
            st.warning("始点と終点の座標を正しく入力してください。")
        else:
            with st.spinner("経路を取得中..."):
                r_type = st.session_state.get("道路種別", "")
                path_str = api_service.fetch_route_coordinates(token, s_coords[0], s_coords[1], e_coords[0], e_coords[1], r_type)
                if path_str:
                    ...
                    st.error("経路の取得に失敗しました。座標や道路種別を確認してください。")
    ...
    st.session_state["規制種別"] = c4_1.selectbox("規制種別", constants.MASTER_DATA["reg_types"], index=safe_index(constants.MASTER_DATA["reg_types"], st.session_state.get("規制種別")))
    ...
    st.session_state["延長_Km"] = c4_4.text_input("規制延長(km)", value=st.session_state.get("延長_Km", ""))
    c4_5, c4_6, c4_7, c4_8 = st.columns(4)
    st.session_state["規制開始_日時"] = c4_5.text_input("開始日時", value=st.session_state.get("規制開始_日時", ""))
    st.session_state["規制開始_内容"] = c4_6.text_input("開始内容", value=st.session_state.get("規制開始_内容", ""))
    st.session_state["規制変更_日時"] = c4_7.text_input("変更日時", value=st.session_state.get("規制変更_日時", ""))
    st.session_state["規制変更_内容"] = c4_8.text_input("変更内容", value=st.session_state.get("規制変更_内容", ""))

    st.markdown('<div class="section-header">5. 影響</div>', unsafe_allow_html=True)
    c5_1, c5_2, c5_3, c5_4, c5_5 = st.columns(5)
    st.session_state["迂回路_有無"] = c5_1.selectbox("迂回路", constants.MASTER_DATA["yes_no"], index=safe_index(constants.MASTER_DATA["yes_no"], st.session_state.get("迂回路_有無")))
    st.session_state["孤立集落_有無"] = c5_2.selectbox("孤立集落", constants.MASTER_DATA["yes_no"], index=safe_index(constants.MASTER_DATA["yes_no"], st.session_state.get("孤立集落_有無")))
    st.session_state["人身_有無"] = c5_3.selectbox("人身被害", constants.MASTER_DATA["yes_no"], index=safe_index(constants.MASTER_DATA["yes_no"], st.session_state.get("人身_有無")))
    st.session_state["物損_有無"] = c5_4.selectbox("物損被害", constants.MASTER_DATA["yes_no"], index=safe_index(constants.MASTER_DATA["yes_no"], st.session_state.get("物損_有無")))
    st.session_state["停電_有無"] = c5_5.selectbox("停電", constants.MASTER_DATA["yes_no"], index=safe_index(constants.MASTER_DATA["yes_no"], st.session_state.get("停電_有無")))
    
    c5_6, c5_7 = st.columns(2)
    st.session_state["迂回路内容"] = c5_6.text_input("迂回路内容", value=st.session_state.get("迂回路内容", ""))
    st.session_state["孤立集落戸数"] = c5_7.text_input("孤立集落戸数・人口", value=st.session_state.get("孤立集落戸数・人口", ""))
    ...
    st.session_state["停電数"] = c5_10.text_input("停電世帯数", value=st.session_state.get("停電世帯数", ""))
