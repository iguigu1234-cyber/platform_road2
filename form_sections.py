...
def safe_get_index(list_obj, val):
    """リスト内に値がない場合に0を返す安全なインデックス取得"""
    try: return list_obj.index(val)
    except: return 0
...
def render_preview_map():
    ...
    st_folium(m, width=850, height=350, key="map_preview")

def render_form(api_service):
    # --- 1. 基本・管理情報 ---
    st.markdown('<div class="section-header">1. 基本・管理情報</div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    st.session_state["タイプ"] = c1.selectbox("タイプ", constants.MASTER_DATA["types"], index=safe_get_index(constants.MASTER_DATA["types"], st.session_state.get("タイプ")))
    ...
    if bc1.button("🔍 道路属性一括取得（始点・終点）", type="primary", use_container_width=True):
    ...
    if bc2.button("🛣️ 経路座標取得 (API9)", use_container_width=True):
    ...
    st.session_state["規制種別"] = c41.selectbox("規制種別", constants.MASTER_DATA["reg_types"], index=safe_get_index(constants.MASTER_DATA["reg_types"], st.session_state.get("規制種別")))
    ...
    st.session_state["迂回路_有無"] = c51.selectbox("迂回路", constants.MASTER_DATA["yes_no"], index=safe_get_index(constants.MASTER_DATA["yes_no"], st.session_state.get("迂回路_有無")))
    st.session_state["孤立集落_有無"] = c52.selectbox("孤立集落", constants.MASTER_DATA["yes_no"], index=safe_get_index(constants.MASTER_DATA["yes_no"], st.session_state.get("孤立集落_有無")))
    st.session_state["人身_有無"] = c53.selectbox("人身被害", constants.MASTER_DATA["yes_no"], index=safe_get_index(constants.MASTER_DATA["yes_no"], st.session_state.get("人身_有無")))
    st.session_state["物損_有無"] = c54.selectbox("物損被害", constants.MASTER_DATA["yes_no"], index=safe_get_index(constants.MASTER_DATA["yes_no"], st.session_state.get("物損_有無")))
    st.session_state["停電_有無"] = c55.selectbox("停電", constants.MASTER_DATA["yes_no"], index=safe_get_index(constants.MASTER_DATA["yes_no"], st.session_state.get("停電_有無")))
